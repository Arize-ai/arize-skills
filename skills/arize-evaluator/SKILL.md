...
- **Provider keys** → when creating an AI integration and the user hasn't provided a provider key, check `.env` for `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`. If not found, **AskQuestion**: "OpenAI/Anthropic API key (needed for AI integrations and evaluators)".
