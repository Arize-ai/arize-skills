...
If `ax` is installed (not `command not found`), always run `ax --version` before investigating further. The version must be `0.11.0` or higher — many errors are caused by an outdated install. If the version is too old, see **Version too old** below.
