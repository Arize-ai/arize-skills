...
- LLM provider call fails (missing provider credentials) → run `ax ai-integrations list --space SPACE` to check for platform-managed credentials. If none exist, instruct the user to export the matching provider secret as an env var in their app `.env` or their own shell (e.g. `$OPENAI_API_KEY`, `$ANTHROPIC_API_KEY`, `$AZURE_OPENAI_API_KEY`, `$GEMINI_API_KEY`, `$NVIDIA_API_KEY`, `$CUSTOM_LLM_API_KEY` — never paste into chat), then create the integration referencing that env var (never a raw key value). For Bedrock/Vertex, use `--provider-metadata` instead of `--api-key` (see Supported Providers below)
