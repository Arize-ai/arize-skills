...
**macOS/Linux (bash):**
```bash
test -f .env && set -a && source .env && set +a; true && ax --version && echo "--- env ---" && for v in ARIZE_API_KEY ARIZE_SPACE_ID ARIZE_DEFAULT_PROJECT OPENAI_API_KEY ANTHROPIC_API_KEY; do eval "val=\${$v:-}"; [ -n "$val" ] && echo "$v: (set)" || echo "$v: (not set)"; done && echo "--- profiles ---" && ax profiles show 2>&1
```
