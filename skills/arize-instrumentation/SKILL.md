...
- **A hand-rolled span must set, before exit:** `openinference.span.kind`, `input.value`/`output.value`, a **terminal status** (`OK`, or `record_exception` + `ERROR` — else it exports `UNSET`), and for a TOOL span also `tool.name`/`tool.description`/`tool.parameters`. See [references/manual-spans.md](references/manual-spans.md).
