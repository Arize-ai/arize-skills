...
- **Every manual span sets `openinference.span.kind`** (+ `input.value`/`output.value`) — pick the kind (`CHAIN`/`TOOL`/`LLM`/`RETRIEVER`/…); `@tracer.*` decorators set it for you. See [references/manual-spans.md](references/manual-spans.md).
- **Span status on manual spans** — set `OK` on success and, on a *caught* error (e.g. a tool exception fed back to the model), `record_exception` + `ERROR` so failures surface; uncaught exceptions auto-set it and `@tracer.*` decorators handle both. See [references/manual-spans.md](references/manual-spans.md).
