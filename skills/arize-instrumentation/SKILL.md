...
**Auto vs manual:** prefer the auto-instrumentor — do **not** hand-roll spans it already covers (duplicates spans, drifts from semconv). Add manual spans only for agent-loop/tool logic no instrumentor sees, or when the stack has no instrumentor at all — keep `register()`/`arize-otel-go` for setup and follow [manual instrumentation](https://arize.com/docs/ax/instrument/manual-instrumentation) with [references/manual-spans.md](references/manual-spans.md).
