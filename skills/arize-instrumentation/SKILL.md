...
3. **Credentials** — User needs an **Arize API Key** and **Space ID**. Check existing `ax` profiles for `ARIZE_API_KEY` and `ARIZE_SPACE` — never read `.env` files:
   ...
   - **Resolve `ARIZE_SPACE` → `ARIZE_SPACE_ID` before generating code.** `ax` accepts a space **name** (e.g. `my-workspace`) or a base64 **ID** (e.g. `U3BhY2U6...`) in `ARIZE_SPACE`. The OTLP collector only accepts the base64 ID — sending a name produces a silent failure (HTTP 200 but no trace ever appears). Run `ax spaces list -o json` and match `$ARIZE_SPACE` against each entry's `name` *or* `id` to find the base64 ID, then export it as `ARIZE_SPACE_ID` in the user's shell before running the instrumented app. Base64 IDs start with `U3BhY2U6`; if `$ARIZE_SPACE` already starts with that prefix, copy it through unchanged.
   - If credentials are not set, instruct the user to set them as environment variables — never embed raw values in generated code. All generated instrumentation code must reference `ARIZE_SPACE_ID` (the resolved base64 ID, never the name): `os.environ["ARIZE_API_KEY"]` / `os.environ["ARIZE_SPACE_ID"]` (Python), `process.env.ARIZE_API_KEY` / `process.env.ARIZE_SPACE_ID` (TypeScript/JavaScript), or `os.Getenv("ARIZE_API_KEY")` / `os.Getenv("ARIZE_SPACE_ID")` (Go).
...
3. If no traces: verify `ARIZE_SPACE_ID` (the resolved base64 ID, not the `ax` name) and `ARIZE_API_KEY`, ensure tracer is initialized before instrumentors and clients, check connectivity to `otlp.arize.com:443`, and inspect app/runtime exporter logs so you can tell whether spans are being emitted locally but rejected remotely. For debug set `GRPC_VERBOSITY=debug` or pass `log_to_console=True` to `register()`. Common gotchas: (a) missing project name resource attribute causes HTTP 500 rejections — `service.name` alone is not enough; Python: pass `project_name` to `register()`; TypeScript: set `"model_id"` or `SEMRESATTRS_PROJECT_NAME` on the resource; Go: add `attribute.String("openinference.project.name", "my-app")` to `resource.New(...)`; (b) CLI/script processes exit before OTLP exports flush — call `provider.force_flush()` then `provider.shutdown()` (Python/TS) or `tp.Shutdown(ctx)` (Go) before exit; (c) CLI-visible spaces/projects can disagree with a collector-targeted space ID — report the mismatch instead of silently rewriting credentials; (d) `ARIZE_SPACE` holds a space *name* (not a base64 ID) and was passed to the OTLP collector as `space_id` — the collector returns HTTP 200 but no trace lands because names aren't resolved. Resolve to the base64 ID via `ax spaces list -o json` and export `ARIZE_SPACE_ID` before re-running.
