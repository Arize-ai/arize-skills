...
**Auto vs manual:** prefer the auto-instrumentor — do **not** hand-roll spans it already covers (duplicates spans, drifts from semconv). Add manual spans only for logic no instrumentor sees, or when the stack has no instrumentor at all. **The OpenAI and Anthropic SDK instrumentors trace only the LLM API call — a raw-SDK app with its own tool-calling loop MUST add manual CHAIN + TOOL spans** or tool inputs/outputs never appear (framework instrumentors like LangChain/LangGraph/OpenAI Agents SDK already cover tools). Keep `register()`/`arize-otel-go` for setup; see [references/manual-spans.md](references/manual-spans.md) and [manual instrumentation](https://arize.com/docs/ax/instrument/manual-instrumentation).
