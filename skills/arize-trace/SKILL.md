...
## List Traces: `ax traces list`

Lightweight, no-download listing of traces in a project — use this to preview which traces match a filter before running the heavier `ax traces export`.

```bash
ax traces list PROJECT --space SPACE --filter "status_code = 'ERROR'" -l 20
```

Supports `--start-time`/`--end-time`, `--filter`, `--limit` (default 15), and `--cursor` for pagination. Table output by default; pass `-o json` for machine-readable output.
...
For the full OpenInference semantic-conventions column reference (prompts/LLM I/O, cost, tool/retriever/reranker spans, session metadata, errors, annotations, embeddings), see [references/span-columns.md](references/span-columns.md).
