...
6. **Apply it:**
   ...
   ```
7. **Report the result to the user**, including anything you skipped (see Rules below) and the two limitations above if they weren't already mentioned. A direct link needs the deep-link path — see the "Unverified assumptions" note in [references/graphql.md](references/graphql.md) before promising a working URL; prefer the **arize-link** skill's project link, or `dashboard.py verify --dashboard DASHBOARD_ID` to confirm the dashboard exists, until that path is confirmed against a live app.
...
|-----------|-----------------|------------|
| `overview` | General "give me a dashboard for this project" request, no specific angle stated | Arize template `generativeLlmModelV2` ("Tracing Project Overview"), augmented with one statistic per discovered eval |
...
| `eval-health` | User asks about eval scores, eval health, or quality over time | Fully composed — no Arize template covers this. Per discovered eval: a score-over-time line chart, a summary statistic, and a label-distribution panel, behind a full-width text header |
...
Full panel-by-panel definitions, including why each blueprint is shaped the way it is, are in [references/blueprints.md](references/blueprints.md).
...
- **Never hand-write `gridPosition`.** The spec format uses `row`/`col`/`width`/`height` (1-indexed on a 12-column grid); `dashboard.py` converts these to Arize's `[rowStart, colStart, rowEnd, colEnd]` array internally. If you omit all four, the widget is left unplaced and the backend auto-places it — do this for widgets added to a templated dashboard (`overview`, `cost-latency`); see the placement-collision note in [references/graphql.md](references/graphql.md).
...
```
dashboard.py discover --project PROJECT_ID [--days N] [--profile P] [--home DIR]
dashboard.py apply    --spec SPEC.json [--dry-run] [--profile P] [--home DIR]
dashboard.py verify   --dashboard DASHBOARD_ID [--profile P] [--home DIR]
dashboard.py list     --space SPACE_ID [--profile P] [--home DIR]
dashboard.py delete   --dashboard DASHBOARD_ID --confirm [--profile P] [--home DIR]
```

All commands print JSON to stdout (or, for `delete`, a one-line confirmation) and exit non-zero with a plain error message on stderr on failure — no tracebacks are surfaced.
...
| `LayoutError` on `apply` | Two placed widgets overlap, or a widget's `row`/`col` is out of the 12-column grid — see [references/spec-format.md](references/spec-format.md) |
