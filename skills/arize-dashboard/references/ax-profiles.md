...
```bash
ax projects list --space SPACE -p work -o json
```
...
## GraphQL uses the same API key

Programmatic dashboard GraphQL expects header **`x-api-key`** with the same service key as `ax`. Use **`$ARIZE_API_KEY`** in `curl` only when it is already exported in the shell — do not read `.env` files to obtain it.
