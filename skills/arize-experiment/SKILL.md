...
3. Call the real model API for each example and collect outputs. Use `ax datasets export --stdout` to pipe examples directly into an inference script:
   ...
   Write `infer.py` to read examples from stdin, call the target model, and write runs JSON to stdout. Start from the template at [references/infer-template.py](references/infer-template.py) — first inspect the exported dataset JSON to find the correct input field name, then uncomment the provider block the user wants. Install the SDK and set the API key env var before running; if the API isn't reachable, stop and tell the user.
